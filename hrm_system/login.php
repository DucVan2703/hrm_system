...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        $db = getDB();
        $stmt = $db->prepare("SELECT tk.*, nv.ho_ten FROM tai_khoan tk LEFT JOIN nhan_vien nv ON tk.id_nhan_vien = nv.id WHERE tk.ten_dang_nhap = ? AND tk.trang_thai = 1");
        ...
        if ($user && password_verify($password, $user['mat_khau'])) {
            ...
            $db->prepare("UPDATE tai_khoan SET lan_dang_nhap_cuoi = NOW() WHERE id = ?")->execute([$user['id']]);
...
<div class="login-page">
  ...
  <div class="login-right">
    <div class="login-logo">
      <img src="assets/images/logo.png" alt="Logo" style="width: 80px; height: 80px; object-fit: contain; margin-bottom: 12px; background: #fff; padding: 4px; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
    ...
    <?php if ($flash): ?>
    <div class="alert alert-<?= $flash['type'] ?>" style="width:100%;margin-bottom:16px;">
      <?= clean($flash['message']) ?>
    </div>
    <?php endif; ?>
    ...
    <form class="login-form" method="POST" action="">
      <div class="form-group">
        ...
        <div style="position:relative;">
          ...
          <input type="text" id="username" name="username" class="form-control" style="padding-left:36px;" placeholder="Nhập tên đăng nhập" value="<?= clean($_POST['username'] ?? '') ?>" required autofocus>
        </div>
      </div>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
        ...
        <a href="forgot-password.php" style="font-size:13px;color:var(--primary);font-weight:500;">Quên mật khẩu?</a>
      </div>
